...
- **Multi-endpoint monitoring** — concurrently verifies:
  ...
  2. `Model: Qwen 3.8 27B (BF16)` — *(Testing concluded at 09:00 today [ref: Discord message](https://discord.com/channels/826099393694400574/1512469795218653417/1541558041692872745); ready for re-activation)*
...
- **Personal API Key Quota Checker (100% Zero-Knowledge & Rate-Limit Protected)** — allows community developers to inspect their own API Key quotas (concurrency, TPM, and budget headroom) in real-time without storing keys. The key is never logged or saved to any database, transmitted over HTTPS through a stateless in-memory Edge Function protected by IP-based sliding window rate limiting (10 req/min/IP) and a 3-second UI anti-spam cooldown, with a built-in Terminal cURL generator for local testing.
